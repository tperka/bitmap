...
typedef struct tagBITMAPINFOHEADER
{
	...
	int biWidth;  //specifies width in pixel_24bits
	int biHeight;  //species height in pixel_24bits
	unsigned short biPlanes; //specifies the number of color planes, must be 1
	unsigned short biBitCount; //specifies the number of bit per pixel_24bit
	unsigned int biCompression;//spcifies the type of compression
	unsigned int biSizeImage;  //size of image in bytes
	int biXPelsPerMeter;  //number of pixel_24bits per meter in x axis
	int biYPelsPerMeter;  //number of pixel_24bits per meter in y axis
